...
function hitTest(x,y){const thr=0.18/zoom;
  for(let i=state.texts.length-1;i>=0;i--){const t=state.texts[i];c.font=`600 ${t.size||14}px 'Inter'`;const w=c.measureText(t.text).width/scl(),h=(t.size||14)/scl(),pad=8/scl();if(x>=t.x-pad&&x<=t.x+w+pad&&y>=t.y-pad&&y<=t.y+h+pad)return{kind:'text',id:t.id};}
