...
function hitTest(x,y){const thr=0.18/zoom;
  for(let i=state.texts.length-1;i>=0;i--){const t=state.texts[i],m=textMetrics(t),pad=8/scl(),a=-(t.angle||0);
    const cx=t.x+m.w/2,cy=t.y+m.h/2,dx=x-cx,dy=y-cy,lx=dx*Math.cos(a)-dy*Math.sin(a),ly=dx*Math.sin(a)+dy*Math.cos(a);
    if(Math.abs(lx)<=m.w/2+pad&&Math.abs(ly)<=m.h/2+pad)return{kind:'text',id:t.id};}
